...
def point_sample(feature_map, point_coords, **kwargs):
    """
    A wrapper around :function:`torch.nn.functional.grid_sample` to support 3D point_coords tensors.
    Unlike :function:`torch.nn.functional.grid_sample` it assumes `point_coords` to lie inside
    [0, 1] x [0, 1] square.
    Args:
        feature_map (Tensor): A tensor of shape (N, C, H, W) that contains features map on a H x W grid.
    ...
    output = f.grid_sample(feature_map, 2.0 * point_coords - 1.0, **kwargs)
    if add_dim:
        output = output.squeeze(3)
    return output


def generate_regular_grid_point_coords(num_grids, side_size, device):
    """
    Generate regular square grid of points in [0, 1] x [0, 1] coordinate space.
    Args:
        num_grids (int): The number of grids to sample, one for each region.
    ...
    return r.view(1, -1, 2).expand(num_grids, -1, -1)
...
def get_uncertain_point_coords_with_randomness(
    ...
    point_logits = point_sample(coarse_logits, point_coords)  # Had: (, align_corners=False) as kwarg, unexpected
...
def get_uncertain_point_coords_on_grid(uncertainty_map, num_points):
    ...
    """
    if isinstance(uncertainty_map.shape[0], int):  # I have no clue what's going on here
        r, _, h, w = uncertainty_map.shape
        h_step = 1 / float(h)
        w_step = 1 / float(w)
    else:
        r = uncertainty_map.shape[0].to(uncertainty_map.device)
        h = uncertainty_map.shape[2].to(uncertainty_map.device)
        w = uncertainty_map.shape[3].to(uncertainty_map.device)
        h_step = 1 / h.float()
        w_step = 1 / w.float()
