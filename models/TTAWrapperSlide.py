...
class TTAWrapperSlide(TTAWrapper):
    def __init__(self,
                 model,
                 scale_list,
                 flip=True,
        ...
        self.crop_size = crop_size if crop_size else [512,512]
        ...
        for s in self.scales:
            for f in flips:
                self.image_scales.append(((int(img_scale[0]*s), int(img_scale[1]*s)),s))
                self.image_flips.append(f)
        printlog(f'Sliding window : strides : {self.strides} crop_size {self.crop_size} image_scales: {self.image_scales}')
    ...
    def multi_scale_aug(self, image, new_shape):
        new_h, new_w = new_shape
        image = cv2.resize(image, (new_w, new_h), interpolation=cv2.INTER_LINEAR)
        return image

    def slide_infer(self, new_img, preds, count, scale, flip):
        stride_h = int(self.strides[0])
        stride_w = int(self.strides[1])
        new_h, new_w = new_img.shape[:-1]
        rows = int(np.ceil((new_h - self.crop_size[0]) / stride_h)) + 1
        cols = int(np.ceil((new_w - self.crop_size[1]) / stride_w)) + 1
        ...
        for r in range(rows):
            for c in range(cols):
                ...
                w1 = min(w0 + self.crop_size[1], new_w)  # x2
                h0 = max(int(h1 - self.crop_size[0]), 0)  # y1
                w0 = max(int(w1 - self.crop_size[1]), 0)  # x1
                crop_img = new_img[h0:h1, w0:w1, :]
                crop_img = crop_img.transpose((2, 0, 1))
                crop_img = np.expand_dims(crop_img, axis=0)
                crop_img = torch.from_numpy(crop_img)
                pred = self.inference(crop_img.cuda(), flip=flip, scale=scale, id_=id_)
                id_ += 1
                # print(w0, preds.shape[3] - w1, int(h0), preds.shape[2] - h1)
    ...
    def forward(self, x):
        ...
        for flip, (shape, scale) in zip(self.image_flips, self.image_scales) :
            new_img = self.multi_scale_aug(image, new_shape=shape)
            ...
            preds, count = self.slide_infer(new_img, preds, count, scale, flip)
            ...
            final_pred += preds
        if self.debug:
            to_comb_image(un_normalise(x[0]), torch.argmax(final_pred[0], 0), None, 1, 'ADE20K', save=f'final.png')
...
if __name__ == '__main__':
    ...
    with torch.no_grad():
        ...
        tta_model = TTAWrapperSlide(model, scale_list=[0.5, 1], crop_size=(512, 512),
                                    strides=(326, 326), debug=True)  #  [0.75, 1.25, 1.5, 1.75, 2, 1.0]
        tta_model.cuda()
        tta_model.eval()
        x = T(img)
        # x  = x.cuda().float()
