    ...
    def forward_step(self, img, lbl, **kwrargs):
        ...
        if isinstance(self.loss, LossWrapper):
            if self.return_features:
                ...
                loss = self.loss(output, lbl.long(), deep_features=proj_features, interm_prediction=interm_output, epoch=self.epoch, skip_mem_update=skip_mem_update)
            else:
                interm_output, output = self.model(img.float())
                proj_features = None
                loss = self.loss(output, lbl.long(), interm_prediction=interm_output, epoch=self.epoch)
        ...
        ret['output'] = output
        ret['interm_output'] = interm_output
